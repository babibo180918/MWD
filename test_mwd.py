...
def data_generator(filepaths, labels, bs):
    ...
    while True:
        ...
        for i in range(0,numOfBatch):
            ...
            batch_X = np.array([cv2.resize(cv2.imread(file_name), (cf.WIDTH, cf.HEIGHT)) for file_name in filepaths_batch])/255.0 - 0.5
...
# load dataset
inputs = np.load(cf.out_mixed_balanced_dataset, allow_pickle=True)
